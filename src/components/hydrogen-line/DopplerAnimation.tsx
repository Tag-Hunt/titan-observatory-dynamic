...
/**
 * Lucide's "cloud" glyph, inlined. The app imports this from lucide-react; it
 * is the only icon the animation uses, so the path is carried here rather than
 * adding the dependency. Rendered as a nested <svg> so the x/y/width/height
 * props position it in the parent's user space, exactly as lucide's does.
 */
function Cloud({ x, y, width, height, fill, fillOpacity, color, strokeWidth = 2, opacity }: CloudProps) {
  return (
    <svg
      x={x}
      y={y}
